...
def test_process_payment_cod(client):
    r = client.post(
        "/payments", json={"order_id": 1, "user_id": 1, "amount": 1500, "method": "cod"}
    )
...
def test_process_payment_invalid_method(client):
    r = client.post(
        "/payments", json={"order_id": 1, "amount": 100, "method": "bitcoin"}
    )
    assert r.status_code == 400
...
def test_wallet(client):
    r = client.post(
        "/payments", json={"order_id": 6, "amount": 500, "method": "wallet"}
    )
    assert r.status_code == 201
