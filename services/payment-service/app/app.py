...
def get_payments():
    ...
    conn.close()
    return (
        jsonify({"payments": [row_to_dict(r) for r in rows], "total": len(rows)}),
        200,
    )
...
def process_payment():
    ...
    conn = get_db()
    cur = conn.execute(
        """INSERT INTO payments (order_id,user_id,amount,method,card_last4,upi_id,bank_name,status,transaction_id,created_at,updated_at)
                          VALUES (?,?,?,?,?,?,?,?,?,?,?)""",
        (
            data["order_id"],
            data.get("user_id", 0),
    ...
    conn.commit()
    payment = row_to_dict(
        conn.execute("SELECT * FROM payments WHERE id=?", (cur.lastrowid,)).fetchone()
    )
    conn.close()
    return (
        jsonify(
                ...
                **payment,
                "message": (
                    ...
                    if status == "success"
                        ...
                        "COD order placed" if status == "pending" else "Payment failed"
                    )
                ),
            }
        ),
...
def refund(pid):
    ...
    conn.close()
    return (
        jsonify({**row_to_dict(row), "message": "Refund initiated successfully"}),
        200,
    )
...
def summary():
    ...
    by_method = conn.execute(
        'SELECT method,COUNT(*),COALESCE(SUM(amount),0) FROM payments WHERE status="success" GROUP BY method'
    ).fetchall()
    conn.close()
    return (
        jsonify(
            {
                ...
                "refunded": refunded,
                "by_method": [
                    {"method": r[0], "count": r[1], "revenue": round(r[2], 2)}
                    for r in by_method
                ],
            }
        ),
