...
def test_get_order_by_id(client):
    r = client.post(
        "/orders",
        json={"user_id": 1, "items": [{"product_id": 1, "price": 1000, "quantity": 2}]},
    )
    oid = r.get_json()["id"]
...
def test_invalid_status(client):
    r = client.post(
        "/orders", json={"user_id": 1, "items": [{"price": 100, "quantity": 1}]}
    )
    oid = r.get_json()["id"]
...
def test_filter_by_user(client):
    client.post(
        "/orders", json={"user_id": 42, "items": [{"price": 100, "quantity": 1}]}
    )
