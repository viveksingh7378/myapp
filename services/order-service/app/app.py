...
def row_to_dict(row):
    ...
    try:
        ...
        d['items'] = []
    try:
        d['address'] = json.loads(d.get('address', '{}'))
    except:
        d['address'] = {}
    return d
...
def order_stats():
    ...
    revenue = conn.execute(
        'SELECT COALESCE(SUM(total),0) FROM orders WHERE status NOT IN ("cancelled","returned")').fetchone()[0]
    by_status = conn.execute(
        'SELECT status,COUNT(*) FROM orders GROUP BY status').fetchall()
    today = conn.execute(
        'SELECT COUNT(*) FROM orders WHERE date(created_at)=date("now")').fetchone()[0]
    conn.close()
